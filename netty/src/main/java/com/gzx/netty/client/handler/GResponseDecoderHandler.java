 ...
 * @Version V1.0
 */
public class GResponseDecoderHandler extends AbstractGResponseHandler {
    ...
    private long gResponseLength;

    private HeaderParser headerParser;
    private ParamtersParser paramtersParser;

    public GResponseDecoderHandler() {
    ...
    protected void decodeGRequest(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) throws Exception {
        ...
        switch (currentState) {
            case READ_HEADER: try {
                ...
                gResponseHeader = createMessage(initialLine);

                gResponseLength = gResponseHeader.getLength();
            ...
            case READ_BODY: try {
                ...
                gResponseHeader.setDecoderResult(DecoderResult.SUCCESS);
                out.add(gResponseHeader);
            ...
            case READ_FIXED_LENGTH_CONTENT: {
                ...
                if (readLimit > gResponseLength) {
                    readLimit = (int) gResponseLength;
                }
                // 返回一个新的ByteBuf
                ByteBuf content = buffer.readRetainedSlice(readLimit);
                gResponseLength -= readLimit;

                if (gResponseLength == 0) {
                    // Read all content.
                    GResponseLastContent gResponseLastContent = new GResponseLastContent(content);
                    gResponseLastContent.setDecoderResult(DecoderResult.SUCCESS);
                    out.add(gResponseLastContent);
                    resetGRequest();
                } else {
    ...
    private State readParamters(ByteBuf buffer) {
        GResponseHeader gResponseHeader = this.gResponseHeader;
        Map paramters =  new HashMap() ;
        gResponseHeader.setParamters(paramters);
    ...
    private GResponseHeader rejectMessage(ByteBuf in, Exception cause) {
        currentState = State.BAD_MESSAGE;

        // 跳过已经读取的bytebuf
        in.skipBytes(in.readableBytes());

        if (gResponseHeader == null) {
            gResponseHeader = createRejectMessage();
        }
        gResponseHeader.setDecoderResult(DecoderResult.failure(cause));

        GResponseHeader ret = gResponseHeader;
        gResponseHeader = null;
        return ret;
    }

    private GResponseHeader createRejectMessage() {
        return new GResponseHeader(500, 0);
    }

    private GResponseHeader createMessage(String[] initialLine) {
